...
class TestNewView:
    ...
    def test_new_view_get(self, user_client):
        ...
        assert 'group' in response.context['form'].fields, (
            ...
            'на странице `/new/` есть поле `group`')
        ...
        assert response.context['form'].fields['text'].required, (
            'Проверьте, что в форме `form`'
            'на странице `/new/` поле `text` обязательно'
        )
    ...
    def test_new_view_post(self, user_client, user, group):
        ...
        assert response.status_code in (301, 302), (
            'Проверьте, что со страницы `/new/` после'
            'создания поста перенаправляете на главную страницу'
        )
        post = Post.objects.filter(author=user, text=text, group=group).first()
        assert post is not None, (
            'Проверьте, что вы сохранили новый пост'
            'при отправки формы на странице `/new/`')
        assert response.url == '/', (
        ...
        assert response.status_code == 200, (
            'Проверьте, что на странице `/new/` выводите ошибки'
