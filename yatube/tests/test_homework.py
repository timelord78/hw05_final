...
class TestPost:
    ...
    def test_post_model(self):
        ...
        assert pub_date_field is not None, (
            'Добавьте дату и время проведения'
            'события `pub_date` модели `Post`')
        ...
        assert author_field is not None, (
            'Добавьте пользователя, автор который'
            'создал событие `author` модели `Post`')
    ...
    def test_post_admin(self):
        ...
        assert 'author' in admin_model.list_display, (
            'Добавьте `author` для отображения в списке'
...
class TestGroupView:
    ...
    def test_group_view(self, client, post_with_group):
        ...
        assert re.search(
            ...
            'не найден заголовок группы `<h1>{{ название_группы }}</h1>`')
        assert re.search(
            r'<\s*p\s*>\s*' + group.description + r'\s*<\s*\/p\s*>',
            html
        ), (
            'Отредактируйте HTML-шаблон,'
            'не найдено описание группы `<p>{{ описание_группы }}</p>`')
